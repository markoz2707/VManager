...
export const getContainers = async (): Promise<{ HCS: Container[], WMI: Container[] }> => {
    ...
    console.log('Container API Response:', data); // Debug log
...
export const createContainer = async (container: {
    ...
    return fetchApi('/containers', {
        method: 'POST',
        body: JSON.stringify({
            Id: `container-${crypto.randomUUID()}`, // Required by API
            Name: container.name,
            ...
            Mode: container.environmentType === 'HCS' ? 0 : 1 // Convert to enum: HCS=0, WMI=1
        })
    });
};

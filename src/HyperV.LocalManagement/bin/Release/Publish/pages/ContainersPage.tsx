...
const ContainerCard: React.FC<{
    ...
    return (
        <div className="bg-panel-bg border border-panel-border rounded-sm shadow-sm p-4 flex flex-col justify-between transition-transform transform hover:-translate-y-1">
            <div>
                <div className="flex justify-between items-start">
                    <h3 className="text-base font-bold text-gray-800">{container.name}</h3>
                    <div className="flex items-center">
                        <span className={`h-2.5 w-2.5 rounded-full mr-2 ${statusColors[container.state] || 'bg-gray-400'}`}></span>
                        <span className="text-xs font-medium text-gray-600">{container.state}</span>
                    </div>
                </div>
                <div className="text-sm text-gray-600 mt-2">
                    <div>Image: <span className="text-gray-800 font-mono text-xs">{container.image}</span></div>
                    <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                        <span>CPUs: <span className="font-semibold text-gray-800">{container.cpuCount}</span></span>
                        <span>Memory: <span className="font-semibold text-gray-800">{container.memoryMB} MB</span></span>
                    </div>
                </div>
            </div>
            <div className="mt-4 flex space-x-1">
                ...
                <Button size="sm" variant="ghost" className="text-red-600 hover:bg-red-50" disabled={isBusy} onClick={() => onDelete(container.id)} title="Delete">
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={`Create New ${activeTab} Container`}>
                <form onSubmit={handleCreateContainer}>
                    <div className="space-y-4">
                        <div>
                            <label htmlFor="container-name" className="block text-sm font-medium text-gray-700">Container Name</label>
                            <input 
                                type="text" 
                                id="container-name" 
                                value={newContainer.name} 
                                onChange={e => setNewContainer({ ...newContainer, name: e.target.value })} 
                                className="mt-1 block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-primary-500 focus:border-primary-500"
                                required 
                            />
                        </div>
                        <div>
                            <label htmlFor="container-image" className="block text-sm font-medium text-gray-700">Container Image</label>
                            <input 
                                type="text" 
                                id="container-image" 
                        ...
                        <div>
                            <label htmlFor="cpu-count" className="block text-sm font-medium text-gray-700">CPU Count</label>
                            <input 
                                type="number" 
                                id="cpu-count" 
                                min="1" 
                        ...
                        <div>
                            <label htmlFor="memory-mb" className="block text-sm font-medium text-gray-700">Memory (MB)</label>
                            <input 
                                type="number" 
                                id="memory-mb" 
                                step="512" 
                        ...
                        <div>
                            <label htmlFor="storage-size-gb" className="block text-sm font-medium text-gray-700">Storage Size (GB)</label>
                            <input 
                                type="number" 
                                id="storage-size-gb" 
                                step="10" 
